...
#define VoltToPIS                                                              \
  820 / sensorMaxPSI // depends on sensor. For 75psi max VoltToPIS =#include
                     // "Arduino.h"
#define pumpOnV HIGH
...
#define i2c_Address                                                            \
  0x3c                // initialize with the I2C addr 0x3C Typically eBay OLED's
...
#define PinPump1 4 // To relay to turn pump on/off with pressure sensor
#define PinPump2                                                               \
  3 // To relay to turn pump on/off borehole pump to fill tank. Float controlled
...
#define PinFlow2                                                               \
  8 // As above but for system pressure pump after settlement tanks.
#define PinTemp 12    // Temperature sensor to protect from frost. 1wre?
#define PinHeater1 10 // To relay to turn on heating.
...
#define LCErrorSensor                                                          \
  4 // The sensor should have a voltage of 0.5 for 0 PSI. So if less the 0.5
    // volts there is a problem like disconnected sensor etc.
...
byte oledWriteAt(byte n, byte x_char, byte y_line, byte padTo = 0,
                 byte textSize = 1, byte charWidth = 6, byte charHight = 8) {
  if (x_char < 1 or y_line < 1)
    return -1;
  if (padTo > 0) {
    display.fillRect((x_char - 1) * textSize * charWidth,
                     (y_line - 1) * textSize * charHight,
                     padTo * textSize * charWidth, 1 * textSize * charHight,
                     SH110X_WHITE);
  }
  ...
  if ((padTo > 0))
    display.setCursor((x_char - 1) * charWidth * textSize,
                      (y_line - 1) * charHight);
  display.println(n);
  display.display();
  return 0;
...
byte oledWriteAt(float n, byte x_char, byte y_line, byte padTo = 3,
                 byte textSize = 1, byte charWidth = 6, byte charHight = 8) {
  if (x_char < 1 or y_line < 1)
    return -1;
  byte charL;
  ...
  if (padTo > 0) {
    display.fillRect((x_char - 1) * textSize * charWidth,
                     ...
                     1 * textSize * charHight, SH110X_WHITE);
  }
...
void loop() {
  ...
  if (sensorValue < ((minValue) / 4)) {
    ...
    Serial.print(F("Curent Pump Borhole state: ")); Serial.print(PumpStateOn); Serial.print(F(", Pin D4: "));Serial.print(digitalRead(4));
    //
